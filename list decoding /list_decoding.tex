...
\section{Introduction}
	...
	\subsection*{Limitations of List Decoding}
	...
	This alone does not answer the Question~\ref{first}. Theorem~\ref{johnson} along with Lemma~\ref{J_function_bound} gives us an affirmative answer to the Question~\ref{first}.
	...
	Theorem~\ref{johnson} and Lemma~\ref{J_function_bound} imply that for a code of relative distance $\delta>0$, we correct strictly larger fraction of errors than unique decoding in ploynomial time as long as $q$ is at most some polynomial in $n$. These two propositions together also implies an \emph{alphabet free} version of the Johnson Bound.
	...
	\subsection*{Proof of Theorem~\ref{ld_capacity}}
	Rather than proving Theorem~\ref{ld_capacity} directly we prove the following  stronger theorem. In fact, we shall show that a random code is $(\rho, L)$-list decodable with high probability as long as 
	\[
	1 - H_q(\rho) - \frac{1}{L}.
	\]
	...
	\begin{theorem}[List-Decoding Capacity]
		...
		\begin{enumerate}[1.]
			...
			\] \label{1}
			\item For all $(\rho, L)$-list decodable code with rate $R\geq 1 - H_q(\rho) + \varepsilon$, it is necesary that $L\geq q^{\Omega(\varepsilon n)}$.\label{2}
		\end{enumerate}
	\end{theorem}
	\begin{proof}
		...
		\noindent\textit{Proof of \ref{1}.}
			Given a received word $\mathbf{y}\in[q]^n$, and $\mathbf{m_0}, \ldots, \mathbf{m_L}\in[q]^k$, the tuple $(\mathbf{y}, \mathbf{m_0}, \ldots, \mathbf{m_L})$ defines a \emph{bad event} if 
			\[
	...
	\noindent\textit{Proof of \ref{2}}
		 We need to show that there exists $\bfy\in[q]^n$ so that $|C\cap B(\bfy,\rho n)|$ is exponentially large for every $\calc$ of rate $R\geq1-H_q(p)+\varepsilon$. Pick $\bfy\in[q]^n$ uniformly at random and fix a $\bfc\in\calc$. Then
		\begin{align*}
...
\section{List Decoding from Random Errors}
